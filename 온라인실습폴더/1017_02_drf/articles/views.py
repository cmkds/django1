...
def article_list(request):
    if request.method == 'GET':
        ...
        if serializer.is_valid(raise_exception=True): ##raise_exception=True 코드는 밑의 400에러 발생 코드와 같은 기능을 한다.
            serializer.save()
...
def article_detail(request, article_pk):
    ...
    article = get_object_or_404(Article, pk=article_pk)  #get이랑 동작은 똑같은데 못찾았을때 값이 없다는 404예외를 발생시킨다.
